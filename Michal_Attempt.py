    ...
    while len(remaining_cs_classes) > 0 or len(remaining_mt_classes) > 0 or len(remaining_sci_classes) > 0 or len(remaining_wr_classes) > 0 or len(remaining_gen_ed) > 0:
        ...
        for term in term_schedule.keys():
            ...
            while cs_count < max_cs_per_term and iter_cs < len(remaining_cs_classes):
                ...
                # Check if the class is offered in the current term
            ...
            # You can follow similar logic for MT and SCI classes
            # Add MT classes
        ...
        for term, classes in term_schedule.items():
            if classes:
                print(f"{term}: {', '.join(classes)}")

        # Move to the next year
        year += 1
        if year >= 10:
            print(remaining_cs_classes)
            print(remaining_sci_classes)
            print(remaining_wr_classes)
